...
bot.on('message', message => {
    ...
    if (args[0] === config.PREFIX) {
        ...
        switch (args.substring(1, messageArray[0].length)) {
            ...
            case 'roll':
                var userNumber = Number(messageArray[1]);

                message.channel.send('```The dice rolled a: ' + getRandomInt(1, userNumber) + '```');
                break;
            ...
            case 'movie':
                ...
                if (!messageArray[1]) {
                    ...
                    let shortMovieEmbed = new Discord.MessageEmbed()
                    .setColor('#439534')
                    .setTitle('!Movie command is performed as follows')
                    .setDescription('Make sure you do \'!movie ' + '**' + 'name of your movie\'' + '**')
                    message.channel.send(shortMovieEmbed);
                    message.delete({ timeout: 1 }).catch(console.error);
                ...
                request('http://omdbapi.com/?t=' + movieSearchQuery + '&apikey=thewdb', function (error, response, body) {
                    if (!error && response.statusCode == 200) {
                        ...
                        var dataAttributes = [data.Released, 
                                            data.Runtime,
                                            data.Actors, 
                                            data.Plot, 
                                            data.Poster,
                        ...
                        if (data.Response == 'False' && data.Error == 'Movie not found!') {
                            const errorEmbed = new Discord.MessageEmbed()
                                .setColor('#fc0303')
                                .setTitle('Issue with search results. Make sure there\'s no type and try again.');
                            message.channel.send(errorEmbed);

                        } else {
                            var missingAttributes = 0;
                            for(var i = 0; i < dataAttributes.length; i++){
                                if((dataAttributes[i]) === 'N/A'){
                                    missingAttributes = missingAttributes + 1;
                                    
                                }
                            }
                            console.log('missingAttributes is : ', missingAttributes);

                            if (Object.keys(data.Ratings).length === 0 || missingAttributes > 3 ) {   
